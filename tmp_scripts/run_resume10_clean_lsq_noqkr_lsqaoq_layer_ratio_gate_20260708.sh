...
EXP="${EXP:-recipe_resume10_clean_lsq_noqkr_lsqaoq_layer_ratio_gate_20260708}"
...
{
  echo "===== Clean LSQ no-QKR LSQ-AOQ layer-ratio gate $(date '+%F %T') ====="
  echo "QATS=${QATS}"
  echo "DATA=${DATA}"
  echo "OUT=${OUT}/${EXP}"
  echo "RESUME=${RESUME}"
