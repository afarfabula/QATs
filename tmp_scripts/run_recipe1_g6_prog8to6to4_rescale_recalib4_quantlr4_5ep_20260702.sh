...
EXP=recipe1_g6_prog8to6to4_rescale_recalib4_quantlr4_5ep_20260702
...
PYTHONUNBUFFERED=1 python3 "$QATS/qat_launch.py" \
  ...
  --devices 0,1,2,3,4,5,6,7 --nproc-per-node 8 --master-port 30323 --model-type swin \
  ...
  --extra-arg=--progressive-bit-schedule --extra-arg=0:8:8,1:6:6,2:4:4 \
  --extra-arg=--progressive-bit-rescale-lsq \
  --extra-arg=--progressive-bit-recalibrate-epochs --extra-arg=2 \
  --extra-arg=--progressive-bit-recalibrate-batches --extra-arg=4 \
