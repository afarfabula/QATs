...
EXP=swin_t_w4a4_stage2_c32_teacher_attn_loww_dynamic_customtop5_from50_to51_20260701
LOG=/tmp/${EXP}.log
OUT=/tmp/qats_stage2_outputs/stage2_50to51_teacherref_20260701
...
PYTHONUNBUFFERED=1 python3 /mlx_devbox/users/quyanyi/playground/QATs/qat_launch.py \
  ...
  --devices 0,1,2,3,4,5,6,7 --nproc-per-node 8 --master-port 30145 \
  --model-type swin --teacher swin_t --teacher-type swin --teacher-checkpoint "${TEACHER}" --teacher-pretrained \
  --resume "${CKPT}" --epochs 51 --start-epoch 50 --scheduler-epochs 100 \
  ...
  --ref-attn-kl-weight 0.0 --teacher-attn-kl-weight 0.000005 --teacher-attn-kl-warmup-epochs 50 \
  --ref-attn-loss kl_ref \
  --ref-head-mode dynamic_custom_top5:custom:5:2,10:14,5:1,4:1,9:10,6:1,8:4,8:9,11:18,11:4 \
  --ref-warmup-epochs 9999 \
  --trainable-policy head_norm_attn_quant \
