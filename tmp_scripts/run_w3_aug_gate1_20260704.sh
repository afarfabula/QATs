...
PYTHONUNBUFFERED=1 python3 "$QATS/qat_launch.py" \
  ...
  --devices 0,1,2,3,4,5,6,7 --nproc-per-node 8 --master-port 30415 --model-type swin \
  --teacher swin_t --teacher-type swin --teacher-checkpoint "$TEACHER" --teacher-pretrained \
  --epochs 1 --scheduler-epochs 10 --batch-size 256 --workers 8 \
