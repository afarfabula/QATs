...
EXP=swin_t_w4a4_stage1_bs256_kd_noaug_stepsoup_src_1ep_20260630
...
PYTHONUNBUFFERED=1 python3 "$QATS/qat_launch.py" \
  ...
  --output "$OUT" --experiment "$EXP" --devices 0,1,2,3,4,5,6,7 --nproc-per-node 8 --master-port 29986 --model-type swin \
  ...
  --extra-arg=--save_step_checkpoints --extra-arg=--step_checkpoint_warmup_updates --extra-arg=425 --extra-arg=--step_checkpoint_interval --extra-arg=50 --extra-arg=--max_step_checkpoints_to_save --extra-arg=4 \
