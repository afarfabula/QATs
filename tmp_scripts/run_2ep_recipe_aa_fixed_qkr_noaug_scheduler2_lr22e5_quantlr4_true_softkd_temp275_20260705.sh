...
EXP="${EXP:-recipe2ep_aa_fixed_qkr_noaug_scheduler2_lr22e5_quantlr4_true_softkd_temp275_20260705}"
...
{
  echo "===== 2ep recipe AA fixed-QKR no-aug scheduler2 lr2.2e-4 quantlr4 true pure-soft-KD temp2.75 $(date '+%F %T') ====="
  echo "QATS=${QATS}"
  echo "DATA=${DATA}"
  echo "OUT=${OUT}/${EXP}"
...
PYTHONUNBUFFERED=1 python3 "${QATS}/qat_launch.py" \
  ...
  --lr 2.2e-4 --min-lr 1e-5 --weight-decay 0.0 \
