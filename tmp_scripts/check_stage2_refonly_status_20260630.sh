...
log=Path('/tmp/train_swin_t_w4a4_stage2_prevstep_customhead_refonly_from10_to20_fastval_20260630.log')
print('LOG', log.exists(), log.stat().st_size if log.exists() else 0)
if log.exists():
    keys=['Train:','TrainSummary','epoch:','distributed-summary','RefAttnKL','Test:','wall_seconds','Traceback','RuntimeError','ProcessRaised','terminate called','Loaded checkpoint','Enabled EMA','Enabled student']
