...
EXP="${EXP:-recipe2ep_i_fixed_qkr_noaug_b32_scheduler1_quantlr4_20260704}"
...
{
  echo "===== 2ep recipe I fixed-QKR no-aug b32 scheduler1 quantlr4 $(date '+%F %T') ====="
  echo "QATS=${QATS}"
  echo "DATA=${DATA}"
  echo "OUT=${OUT}/${EXP}"
...
PYTHONUNBUFFERED=1 python3 "${QATS}/qat_launch.py" \
  ...
  --epochs 2 --scheduler-epochs 1 --batch-size 32 --workers 8 \
