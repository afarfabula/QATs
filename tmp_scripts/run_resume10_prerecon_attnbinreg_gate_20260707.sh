...
EXP="${EXP:-recipe_resume10_prerecon_attnbinreg_gate_20260707}"
...
BIN_REG_WEIGHT="${BIN_REG_WEIGHT:-2e-5}"
BIN_REG_VARIANCE_WEIGHT="${BIN_REG_VARIANCE_WEIGHT:-1.0}"
BIN_REG_LAYERS="${BIN_REG_LAYERS:-features.5.5.attn,features.7.1.attn}"
...
{
  echo "===== Resume10 pre-QAT feature recon + attention-only BinReg gate $(date '+%F %T') ====="
  echo "QATS=${QATS}"
  echo "DATA=${DATA}"
  echo "OUT=${OUT}/${EXP}"
  echo "RESUME=${RESUME}"
