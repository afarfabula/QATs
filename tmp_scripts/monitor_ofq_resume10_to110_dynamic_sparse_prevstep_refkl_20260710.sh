...
STATUS="${STATUS:-${DOC_DIR}/ofq_resume10_to110_dynamic_sparse_prevstep_refkl_status_20260710.tsv}"
REF_STATUS="${REF_STATUS:-${DOC_DIR}/ofq_resume10_to110_dynamic_sparse_prevstep_refkl_refw_20260710.tsv}"
CONTROLLER_STATUS="${CONTROLLER_STATUS:-${DOC_DIR}/ofq_resume10_to110_dynamic_sparse_prevstep_refkl_controller_20260710.tsv}"
SUMMARY="${SUMMARY:-${DOC_DIR}/ofq_resume10_to110_dynamic_sparse_prevstep_refkl_monitor_summary_20260710.txt}"
...
{
  printf 'timestamp\tcheckpoint\tloss\tacc1\tacc5\tsamples\tdelta_vs_80.5980\tdelta_vs_scheme_c_80.6820\tdelta_vs_original_80.7240\tstatus\n'
  if [[ -f "${LOG}" ]]; then
    awk -v ts="${ts}" -v baseline="${BASELINE}" -v scheme_c="${SCHEME_C_BEST}" -v original="${ORIGINAL_BEST}" '
      ...
      END {
        for (i=1; i<=n; i++) {
          ...
          printf "%s\tcheckpoint-%d\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n", ts, ckpt, a[1], a[2], a[3], a[4], delta_base, delta_scheme, delta_original, status;
        }
      }
    ' "${LOG}"
  fi
...
if [[ -s "${REF_STATUS}" ]]; then
  ...
  pre61_nonzero_refw="$(awk -F '\t' 'NR > 1 && $2+0 <= 60 && $3 != "0.000e+00" && $3+0 != 0 { c++ } END { print c+0 }' "${REF_STATUS}")"
fi
...
if [[ -s "${CONTROLLER_STATUS}" ]]; then
  ...
  controller_pre61_triggers="$(awk -F '\t' 'NR > 1 && $1+0 <= 60 && $14+0 > 0 { c++ } END { print c+0 }' "${CONTROLLER_STATUS}")"
