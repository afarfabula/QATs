...
EXP="${EXP:-recipe4ep_m_featpyramid_aug_fixed_qkr_softkd_t275_20260705}"
...
{
  echo "===== 4ep recipe M public-style W4A4-family feature-pyramid + augmented teacher distill fixed-QKR soft-KD T=2.75 $(date '+%F %T') ====="
  echo "QATS=${QATS}"
  echo "DATA=${DATA}"
  echo "OUT=${OUT}/${EXP}"
...
PYTHONUNBUFFERED=1 python3 "${QATS}/qat_launch.py" \
  ...
  --teacher-feature-output-layers features.3.1,features.5.5,features.7.1 \
