...
SRC_EXP=swin_t_w4a4_stage1_fromscratch_bs256_kd_noaug_strictckpt_100ep_20260702
EXP=swin_t_w4a4_stage1_strict_resume100_to200_bs256_kd_noaug_20260702
CKPT=${OUT}/${SRC_EXP}/checkpoint-100.pth.tar
...
PYTHONUNBUFFERED=1 python3 "$QATS/qat_launch.py" \
  ...
  --devices 0,1,2,3,4,5,6,7 --nproc-per-node 8 --master-port 30282 --model-type swin \
  --teacher swin_t --teacher-type swin --teacher-checkpoint "$TEACHER" --teacher-pretrained \
  --resume "$CKPT" --epochs 200 --scheduler-epochs 200 \
  --batch-size 256 --workers 8 --lr 2e-4 --min-lr 1e-5 --weight-decay 0.0 \
  --epoch-checkpoint-interval 10 --checkpoint-hist 12 \
