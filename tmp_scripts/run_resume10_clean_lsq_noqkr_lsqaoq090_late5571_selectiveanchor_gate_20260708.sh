...
EXP="${EXP:-recipe_resume10_clean_lsq_noqkr_lsqaoq090_late5571_selectiveanchor_gate_20260708}"
...
SELECTIVE_BIN_ANCHOR_WEIGHT="${SELECTIVE_BIN_ANCHOR_WEIGHT:-1e-4}"
SELECTIVE_BIN_ANCHOR_LAYERS="${SELECTIVE_BIN_ANCHOR_LAYERS:-features.5.5.attn.qkv,features.5.5.attn.proj,features.5.5.mlp.fc2,features.7.1.attn.qkv,features.7.1.attn.proj,features.7.1.mlp.fc2}"
...
cmd=(
  ...
  --selective-bin-anchor-weight "${SELECTIVE_BIN_ANCHOR_WEIGHT}"
  --selective-bin-anchor-layers "${SELECTIVE_BIN_ANCHOR_LAYERS}"
  --selective-bin-anchor-capture-update "${SELECTIVE_BIN_ANCHOR_CAPTURE_UPDATE}"
  --selective-bin-anchor-end-update "${SELECTIVE_BIN_ANCHOR_END_UPDATE}"
  --selective-bin-anchor-margin "${SELECTIVE_BIN_ANCHOR_MARGIN}"
...
{
  echo "===== Clean LSQ no-QKR LSQ-AOQ090 late5571 selective-anchor gate $(date '+%F %T') ====="
  echo "QATS=${QATS}"
  echo "DATA=${DATA}"
  echo "OUT=${OUT}/${EXP}"
  echo "RESUME=${RESUME}"
