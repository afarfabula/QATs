...
EXP="${EXP:-recipe2ep_ab_fixed_qkr_noaug_scheduler3_quantlr4_true_softkd_temp275_20260705}"
...
{
  echo "===== 2ep recipe AB fixed-QKR no-aug scheduler3 quantlr4 true pure-soft-KD temp2.75 $(date '+%F %T') ====="
  echo "QATS=${QATS}"
  echo "DATA=${DATA}"
  echo "OUT=${OUT}/${EXP}"
...
PYTHONUNBUFFERED=1 python3 "${QATS}/qat_launch.py" \
  ...
  --epochs 2 --scheduler-epochs 3 --batch-size 64 --workers 8 \
