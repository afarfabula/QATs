...
EXP="${EXP:-recipe2ep_s_fixed_qkr_noaug_scheduler1_quantlr6_true_softkd_temp25_20260705}"
...
{
  echo "===== 2ep recipe S fixed-QKR no-aug scheduler1 quantlr6 true pure-soft-KD temp2.5 $(date '+%F %T') ====="
  echo "QATS=${QATS}"
  echo "DATA=${DATA}"
  echo "OUT=${OUT}/${EXP}"
...
PYTHONUNBUFFERED=1 python3 "${QATS}/qat_launch.py" \
  ...
  --epochs 2 --scheduler-epochs 1 --batch-size 64 --workers 8 \
  --lr 2e-4 --min-lr 1e-5 --weight-decay 0.0 \
  --quant-lr-multiplier 6 \
  --epoch1-acc-gate 77.0 \
  ...
  --pretrained --pretrained-initialized --use-kd --kd-hard-and-soft 0 --teacher-soft-temperature 2.5 \
