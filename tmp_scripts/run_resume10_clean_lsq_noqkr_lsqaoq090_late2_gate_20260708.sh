...
EXP="${EXP:-recipe_resume10_clean_lsq_noqkr_lsqaoq090_late2_gate_20260708}"
...
AOQ_EXPLORE_LAYERS="${AOQ_EXPLORE_LAYERS:-features.6.1.attn.qkv,features.6.1.attn.proj,features.6.1.mlp.fc2,features.7.1.attn.qkv,features.7.1.attn.proj,features.7.1.mlp.fc2}"
...
{
  echo "===== Clean LSQ no-QKR LSQ-AOQ090 late2 gate $(date '+%F %T') ====="
  echo "QATS=${QATS}"
  echo "DATA=${DATA}"
  echo "OUT=${OUT}/${EXP}"
  echo "RESUME=${RESUME}"
