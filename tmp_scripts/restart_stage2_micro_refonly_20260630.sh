...
PYTHONUNBUFFERED=1 python3 "$QATS/qat_launch.py" \\
  ...
  --devices 0,1,2,3,4,5,6,7 --nproc-per-node 8 --master-port 29992 --model-type swin \\
  --resume "$RESUME" --no-resume-opt --epochs 20 --scheduler-epochs 50 \\
  --batch-size 256 --forward-micro-batch-size 64 --workers 8 --lr 5e-5 --min-lr 1e-5 --weight-decay 0.0 --epoch-checkpoint-interval 1 \\
...
tail -n 160 /tmp/${EXP}.runner.log || true
