...
EXP="${EXP:-recipe_resume10_prerecon_qkvrel_gate_20260707}"
...
TEACHER_QKV_REL_WEIGHT="${TEACHER_QKV_REL_WEIGHT:-10}"
TEACHER_QKV_REL_LAYERS="${TEACHER_QKV_REL_LAYERS:-10,11}"
TEACHER_QKV_REL_COMPONENTS="${TEACHER_QKV_REL_COMPONENTS:-q,k,v}"
...
cmd=(
  ...
  --teacher-attn-kl-weight 0
  --teacher-qk-rel-weight 0
  --teacher-qkv-rel-weight "${TEACHER_QKV_REL_WEIGHT}"
  --teacher-qkv-rel-warmup-epochs 0
  --teacher-qkv-rel-layers "${TEACHER_QKV_REL_LAYERS}"
  --teacher-qkv-rel-components "${TEACHER_QKV_REL_COMPONENTS}"
...
if [[ -n "${MAX_TRAIN_UPDATES}" && "${MAX_TRAIN_UPDATES}" != "0" ]]; then
  cmd+=(--extra-arg=--max-train-updates --extra-arg="${MAX_TRAIN_UPDATES}")
...
{
  echo "===== Resume10 pre-QAT feature recon + local teacher QKV relation gate $(date '+%F %T') ====="
  echo "QATS=${QATS}"
  echo "DATA=${DATA}"
  echo "OUT=${OUT}/${EXP}"
  echo "RESUME=${RESUME}"
  ...
  echo "TEACHER_QKV_REL_WEIGHT=${TEACHER_QKV_REL_WEIGHT}"
  echo "TEACHER_QKV_REL_LAYERS=${TEACHER_QKV_REL_LAYERS}"
  echo "TEACHER_QKV_REL_COMPONENTS=${TEACHER_QKV_REL_COMPONENTS}"
