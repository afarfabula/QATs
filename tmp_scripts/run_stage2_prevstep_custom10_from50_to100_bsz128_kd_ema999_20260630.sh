...
python3 /mlx_devbox/users/quyanyi/playground/QATs/qat_launch.py \
  ...
  --devices 0,1,2,3,4,5,6,7 --nproc-per-node 8 --master-port 30015 --model-type swin \
  ...
  --resume "${CKPT}" --no-resume-opt --epochs 100 --start-epoch 50 --scheduler-epochs 100 \
  ...
  --extra-arg=--static-graph --extra-arg=--smoothing --extra-arg=0.0 --extra-arg=--mixup --extra-arg=0.0 \
  --extra-arg=--cutmix --extra-arg=0.0 --extra-arg=--aa --extra-arg=none --extra-arg=--color-jitter --extra-arg=0.0 \
  --extra-arg=--reprob --extra-arg=0.0 --extra-arg=--log-interval --extra-arg=50 --extra-arg=--seed --extra-arg=42 2>&1 | tee "${LOG}"
