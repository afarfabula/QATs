...
EXP="${EXP:-recipe2ep_n_fixed_qkr_noaug_scheduler1_quantlr4_prevkl_20260704}"
...
{
  echo "===== 2ep recipe N fixed-QKR no-aug scheduler1 quantlr4 prevkl $(date '+%F %T') ====="
  echo "QATS=${QATS}"
  echo "DATA=${DATA}"
  echo "OUT=${OUT}/${EXP}"
...
PYTHONUNBUFFERED=1 python3 "${QATS}/qat_launch.py" \
  ...
  --ref-attn-kl-weight 1e-4 --ref-attn-loss kl_ref \
  --ref-head-mode dynamic_custom_top5:oscillating_top10 \
