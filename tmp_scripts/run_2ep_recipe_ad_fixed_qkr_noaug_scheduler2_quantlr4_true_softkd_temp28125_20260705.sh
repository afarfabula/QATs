...
EXP="${EXP:-recipe2ep_ad_fixed_qkr_noaug_scheduler2_quantlr4_true_softkd_temp28125_20260705}"
...
{
  echo "===== 2ep recipe AD fixed-QKR no-aug scheduler2 quantlr4 true pure-soft-KD temp2.8125 $(date '+%F %T') ====="
  echo "QATS=${QATS}"
  echo "DATA=${DATA}"
  echo "OUT=${OUT}/${EXP}"
...
PYTHONUNBUFFERED=1 python3 "${QATS}/qat_launch.py" \
  ...
  --epochs 2 --scheduler-epochs 2 --batch-size 64 --workers 8 \
  ...
  --pretrained --pretrained-initialized --use-kd --kd-hard-and-soft 0 --teacher-soft-temperature 2.8125 \
