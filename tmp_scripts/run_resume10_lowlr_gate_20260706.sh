...
EXP="${EXP:-recipe_resume10_lowlr_gate_a_lr1e5_qm1_20260706}"
...
MASTER_PORT="${MASTER_PORT:-30522}"
EPOCHS="${EPOCHS:-3}"
SCHEDULER_EPOCHS="${SCHEDULER_EPOCHS:-3}"
LR="${LR:-1e-5}"
MIN_LR="${MIN_LR:-5e-6}"
QUANT_LR_MULTIPLIER="${QUANT_LR_MULTIPLIER:-1}"
...
PRE_QAT_FEATURE_RECON_UPDATES="${PRE_QAT_FEATURE_RECON_UPDATES:-}"
PRE_QAT_FEATURE_RECON_LAYERS="${PRE_QAT_FEATURE_RECON_LAYERS:-}"
PRE_QAT_FEATURE_RECON_POLICY="${PRE_QAT_FEATURE_RECON_POLICY:-}"
PRE_QAT_FEATURE_RECON_CONFIDENCE_POWER="${PRE_QAT_FEATURE_RECON_CONFIDENCE_POWER:-}"
PRE_QAT_FEATURE_RECON_WEIGHT_MODE="${PRE_QAT_FEATURE_RECON_WEIGHT_MODE:-}"
PRE_QAT_FEATURE_RECON_ANCHOR_KL_WEIGHT="${PRE_QAT_FEATURE_RECON_ANCHOR_KL_WEIGHT:-}"
PRE_QAT_FEATURE_RECON_ANCHOR_KL_TEMPERATURE="${PRE_QAT_FEATURE_RECON_ANCHOR_KL_TEMPERATURE:-}"
...
if [[ -n "${PRE_QAT_FEATURE_RECON_UPDATES}" && "${PRE_QAT_FEATURE_RECON_UPDATES}" != "0" ]]; then
  cmd+=(--pre-qat-feature-recon-updates "${PRE_QAT_FEATURE_RECON_UPDATES}")
  cmd+=(--pre-qat-feature-recon-layers "${PRE_QAT_FEATURE_RECON_LAYERS:-features.5.5,features.7.1}")
  cmd+=(--pre-qat-feature-recon-policy "${PRE_QAT_FEATURE_RECON_POLICY:-quant}")
  if [[ -n "${PRE_QAT_FEATURE_RECON_CONFIDENCE_POWER}" ]]; then
    cmd+=(--pre-qat-feature-recon-confidence-power "${PRE_QAT_FEATURE_RECON_CONFIDENCE_POWER}")
  fi
  if [[ -n "${PRE_QAT_FEATURE_RECON_WEIGHT_MODE}" ]]; then
    cmd+=(--pre-qat-feature-recon-weight-mode "${PRE_QAT_FEATURE_RECON_WEIGHT_MODE}")
  fi
  if [[ -n "${PRE_QAT_FEATURE_RECON_ANCHOR_KL_WEIGHT}" ]]; then
    cmd+=(--pre-qat-feature-recon-anchor-kl-weight "${PRE_QAT_FEATURE_RECON_ANCHOR_KL_WEIGHT}")
  fi
  if [[ -n "${PRE_QAT_FEATURE_RECON_ANCHOR_KL_TEMPERATURE}" ]]; then
    cmd+=(--pre-qat-feature-recon-anchor-kl-temperature "${PRE_QAT_FEATURE_RECON_ANCHOR_KL_TEMPERATURE}")
  fi
fi
...
{
  echo "===== Phase 1/2 gate: resume10 low-LR public-family W4A4 $(date '+%F %T') ====="
  echo "QATS=${QATS}"
  echo "DATA=${DATA}"
  echo "OUT=${OUT}/${EXP}"
  echo "RESUME=${RESUME}"
  ...
  echo "PRE_QAT_FEATURE_RECON_CONFIDENCE_POWER=${PRE_QAT_FEATURE_RECON_CONFIDENCE_POWER}"
  echo "PRE_QAT_FEATURE_RECON_WEIGHT_MODE=${PRE_QAT_FEATURE_RECON_WEIGHT_MODE}"
  echo "PRE_QAT_FEATURE_RECON_ANCHOR_KL_WEIGHT=${PRE_QAT_FEATURE_RECON_ANCHOR_KL_WEIGHT}"
  echo "PRE_QAT_FEATURE_RECON_ANCHOR_KL_TEMPERATURE=${PRE_QAT_FEATURE_RECON_ANCHOR_KL_TEMPERATURE}"
