...
EXP="${EXP:-recipe_resume10_disagree_prerecon_gate_20260707}"
...
PRE_QAT_FEATURE_RECON_WEIGHT_MODE="${PRE_QAT_FEATURE_RECON_WEIGHT_MODE:-disagreement}"
...
cmd=(
  ...
  --pre-qat-feature-recon-weight-mode "${PRE_QAT_FEATURE_RECON_WEIGHT_MODE}"
...
{
  echo "===== Resume10 disagreement-weighted pre-QAT feature recon gate $(date '+%F %T') ====="
  echo "QATS=${QATS}"
  echo "DATA=${DATA}"
  echo "OUT=${OUT}/${EXP}"
  echo "RESUME=${RESUME}"
