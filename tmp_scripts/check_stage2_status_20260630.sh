#!/usr/bin/env bash
set -euo pipefail
EXP=swin_t_w4a4_stage2_prevstep_customhead_from10_to20_fastval_20260630
...
log=Path('/tmp/train_swin_t_w4a4_stage2_prevstep_customhead_from10_to20_fastval_20260630.log')
if not log.exists():
    print('missing log')
else:
    keys=['TrainSummary','epoch:','distributed-summary','RefAttnKL','AnchorRef','Test:','wall_seconds','Traceback','RuntimeError','ProcessRaised']
