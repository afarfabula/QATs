...
EXP="${EXP:-ofq_resume10_to30_late_prevstep_refkl_20260709}"
...
MASTER_PORT="${MASTER_PORT:-31541}"
REF_HEAD_MODE="${REF_HEAD_MODE:-custom_subset:6:1,8:4,8:9,11:18,11:4}"
REF_WEIGHT_OVERRIDES="${REF_WEIGHT_OVERRIDES:-23:0.0002,25:0.0005,28:0.0002}"
SECONDS=0
...
{
  echo "===== Public-family OFQ resume10->30 + late selective prev-step refKL $(date '+%F %T') ====="
  echo "QATS=${QATS}"
  echo "DATA=${DATA}"
  echo "OUT=${OUT}/${EXP}"
  echo "LOG=${LOG}"
...
PYTHONUNBUFFERED=1 python3 "${QATS}/qat_launch.py" \
  ...
  --ref-warmup-epochs 23 \
  --ref-attn-kl-drop-prob 0.25 \
