...
EXP="${EXP:-recipe4ep_n_actcurr_aug_fixed_qkr_softkd_t275_20260705}"
...
{
  echo "===== 4ep recipe N public-style W4A4-family activation curriculum W4A8->W4A4 + feature-output + augmented teacher distill fixed-QKR soft-KD T=2.75 $(date '+%F %T') ====="
  echo "QATS=${QATS}"
  echo "DATA=${DATA}"
  echo "OUT=${OUT}/${EXP}"
...
PYTHONUNBUFFERED=1 python3 "${QATS}/qat_launch.py" \
  ...
  --extra-arg=--progressive-bit-schedule --extra-arg=0:4:8,1:4:4 \
