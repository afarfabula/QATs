...
# normal 1-epoch run only: no soup, no step checkpoints, no resume
EXP=swin_t_w4a4_stage1_bs256_kd_noaug_attncopyfix_1ep_fastval_20260630
...
PYTHONUNBUFFERED=1 python3 "$QATS/qat_launch.py" \
  ...
  --master-port 29987 \
  --model-type swin \
  --teacher swin_t --teacher-type swin \
  --teacher-checkpoint "$TEACHER" --teacher-pretrained \
  --epochs 1 --scheduler-epochs 50 \
