...
EXP="${EXP:-recipe_resume10_prerecon_freezeact_exceptlate_gate_20260707}"
...
TRAINABLE_POLICY="${TRAINABLE_POLICY:-freeze_act_except_layers}"
...
cmd=(
  ...
  --trainable-policy "${TRAINABLE_POLICY}"
  --trainable-policy-freeze-act-except-layers "${TRAINABLE_POLICY_FREEZE_ACT_EXCEPT_LAYERS}"
...
{
  echo "===== Resume10 pre-QAT feature recon + freeze non-late activation quant/shift gate $(date '+%F %T') ====="
  echo "QATS=${QATS}"
  echo "DATA=${DATA}"
  echo "OUT=${OUT}/${EXP}"
  echo "RESUME=${RESUME}"
