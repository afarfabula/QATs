...
MASTER_PORT="${MASTER_PORT:-31841}"
DOC_DIR="${DOC_DIR:-/mlx_devbox/users/quyanyi/playground/QATs/docs}"
CONTROLLER_TSV="${CONTROLLER_TSV:-${DOC_DIR}/ofq_resume10_to210_late_sparse_prevstep_refkl_controller_20260712.tsv}"
DRY_RUN="${DRY_RUN:-0}"
SECONDS=0
...
{
  echo "===== OFQ resume10->210 late sparse prev-step refKL $(date '+%F %T') ====="
  echo "QATS=${QATS}"
  echo "DATA=${DATA}"
  echo "OUT=${OUT}/${EXP}"
  echo "LOG=${LOG}"
...
CMD=(python3 "${QATS}/qat_launch.py"
  ...
  --epochs 210 --scheduler-epochs 210
  --batch-size 64 --workers 8 --lr 1.5e-5 --min-lr 5e-6 --weight-decay 0.0
  --epoch-checkpoint-interval 1 --checkpoint-hist 210
  ...
  --dynamic-kl-start-epoch 91
  --dynamic-kl-observe-until-epoch 90
  ...
  --dynamic-kl-drop-threshold 0.08
  --dynamic-kl-strong-drop-threshold 0.16
  --dynamic-kl-default-weight 0.00001
  --dynamic-kl-strong-weight 0.00002
  --dynamic-kl-max-weight 0.00002
  --dynamic-kl-cooldown-epochs 7
  --dynamic-kl-window-epochs 12
  --dynamic-kl-max-pulses-per-window 2
  --dynamic-kl-controller-tsv "${CONTROLLER_TSV}"
  --dynamic-kl-prior-source offline_attn_relation_late_stabilizer_from_original10to110_and_dynamic10to110_20260712
