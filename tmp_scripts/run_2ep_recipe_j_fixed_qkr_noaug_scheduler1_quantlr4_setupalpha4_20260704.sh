...
EXP="${EXP:-recipe2ep_j_fixed_qkr_noaug_scheduler1_quantlr4_setupalpha4_20260704}"
...
{
  echo "===== 2ep recipe J fixed-QKR no-aug scheduler1 quantlr4 setupalpha4 $(date '+%F %T') ====="
  echo "QATS=${QATS}"
  echo "DATA=${DATA}"
  echo "OUT=${OUT}/${EXP}"
...
PYTHONUNBUFFERED=1 python3 "${QATS}/qat_launch.py" \
  ...
  --amp --amp-dtype bf16 \
  --extra-arg=--setup-alpha-batches --extra-arg=4 \
