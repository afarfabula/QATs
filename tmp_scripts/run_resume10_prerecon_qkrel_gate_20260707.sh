...
EXP="${EXP:-recipe_resume10_prerecon_qkrel_gate_20260707}"
...
TEACHER_QK_REL_WEIGHT="${TEACHER_QK_REL_WEIGHT:-1000}"
...
cmd=(
  ...
  --teacher-qk-rel-weight "${TEACHER_QK_REL_WEIGHT}"
...
{
  echo "===== Resume10 pre-QAT feature recon + scaled teacher QK relation gate $(date '+%F %T') ====="
  echo "QATS=${QATS}"
  echo "DATA=${DATA}"
  echo "OUT=${OUT}/${EXP}"
  echo "RESUME=${RESUME}"
  ...
  echo "TEACHER_QK_REL_WEIGHT=${TEACHER_QK_REL_WEIGHT}"
  echo "TEACHER_ATTN_KL_WEIGHT=0"
