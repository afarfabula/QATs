...
EXP="${EXP:-recipe_resume10_prerecon_postrefresh20_gate_20260707}"
...
POST_EPOCH_FEATURE_RECON_UPDATES="${POST_EPOCH_FEATURE_RECON_UPDATES:-20}"
...
cmd=(
  ...
  --post-epoch-feature-recon-updates "${POST_EPOCH_FEATURE_RECON_UPDATES}"
...
{
  echo "===== Resume10 Phase 2B pre-QAT feature recon + post-epoch refresh gate $(date '+%F %T') ====="
  echo "QATS=${QATS}"
  echo "DATA=${DATA}"
  echo "OUT=${OUT}/${EXP}"
  echo "RESUME=${RESUME}"
  ...
  echo "POST_EPOCH_FEATURE_RECON_UPDATES=${POST_EPOCH_FEATURE_RECON_UPDATES}"
