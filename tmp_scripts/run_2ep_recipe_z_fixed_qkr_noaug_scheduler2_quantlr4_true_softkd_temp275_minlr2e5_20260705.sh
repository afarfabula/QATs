...
EXP="${EXP:-recipe2ep_z_fixed_qkr_noaug_scheduler2_quantlr4_true_softkd_temp275_minlr2e5_20260705}"
...
{
  echo "===== 2ep recipe Z fixed-QKR no-aug scheduler2 quantlr4 true pure-soft-KD temp2.75 minlr2e-5 $(date '+%F %T') ====="
  echo "QATS=${QATS}"
  echo "DATA=${DATA}"
  echo "OUT=${OUT}/${EXP}"
...
PYTHONUNBUFFERED=1 python3 "${QATS}/qat_launch.py" \
  ...
  --lr 2e-4 --min-lr 2e-5 --weight-decay 0.0 \
