...
EXP="${EXP:-recipe3ep_c_quantfirst100_fixed_qkr_softkd_t275_20260705}"
...
{
  echo "===== 3ep recipe C public-style W4A4-family quant-first100 fixed-QKR soft-KD T=2.75 $(date '+%F %T') ====="
  echo "QATS=${QATS}"
  echo "DATA=${DATA}"
  echo "OUT=${OUT}/${EXP}"
...
PYTHONUNBUFFERED=1 python3 "${QATS}/qat_launch.py" \
  ...
  --trainable-policy-update-overrides 100:all \
