...
EXP=swin_t_w4a4_stage1_resume100_strict_smoke_1update_20260702
...
PYTHONUNBUFFERED=1 python3 "$QATS/qat_launch.py" \
  ...
  --devices 0,1,2,3,4,5,6,7 --nproc-per-node 8 --master-port 30281 --model-type swin \
  --teacher swin_t --teacher-type swin --teacher-checkpoint "$TEACHER" --teacher-pretrained \
  --resume "$CKPT" --epochs 101 --start-epoch 100 --scheduler-epochs 100 \
