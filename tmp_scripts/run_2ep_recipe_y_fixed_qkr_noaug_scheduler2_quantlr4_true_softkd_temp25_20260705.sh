...
EXP="${EXP:-recipe2ep_y_fixed_qkr_noaug_scheduler2_quantlr4_true_softkd_temp25_20260705}"
...
{
  echo "===== 2ep recipe Y fixed-QKR no-aug scheduler2 quantlr4 true pure-soft-KD temp2.5 $(date '+%F %T') ====="
  echo "QATS=${QATS}"
  echo "DATA=${DATA}"
  echo "OUT=${OUT}/${EXP}"
