#!/usr/bin/env bash
set -euo pipefail
# Kill hung stage2 process group if present.
for p in $(pgrep -f 'swin_t_w4a4_stage2_prevstep_customhead_from10_to20_fastval_20260630|swin_t_w4a4_stage2_prevstep_customhead_nokd_from10_to20_fastval_20260630' || true); do
  pg=$(ps -o pgid= -p "$p" | tr -d ' ' || true)
  if [ -n "$pg" ]; then
    echo "Killing old stage2 pid=$p pgid=$pg"
    kill -TERM -"$pg" || true
  fi
done
sleep 8
pgrep -af 'swin_t_w4a4_stage2_prevstep_customhead_from10_to20|swin_t_w4a4_stage2_prevstep_customhead_nokd_from10_to20|qat_launch.py' || true
...
PYTHONUNBUFFERED=1 python3 "$QATS/qat_launch.py" \\
  ...
  --devices 0,1,2,3,4,5,6,7 --nproc-per-node 8 --master-port 29990 --model-type swin \\
