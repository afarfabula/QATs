...
EXP="${EXP:-recipe2ep_ac_fixed_qkr_noaug_scheduler2_quantlr4_true_softkd_temp2625_20260705}"
...
{
  echo "===== 2ep recipe AC fixed-QKR no-aug scheduler2 quantlr4 true pure-soft-KD temp2.625 $(date '+%F %T') ====="
  echo "QATS=${QATS}"
  echo "DATA=${DATA}"
  echo "OUT=${OUT}/${EXP}"
...
PYTHONUNBUFFERED=1 python3 "${QATS}/qat_launch.py" \
  ...
  --pretrained --pretrained-initialized --use-kd --kd-hard-and-soft 0 --teacher-soft-temperature 2.625 \
