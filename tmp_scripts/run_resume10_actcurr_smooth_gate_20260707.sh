...
EXP="${EXP:-recipe_resume10_actcurr_w4a8_w4a6_w4a4_20260707}"
...
MASTER_PORT="${MASTER_PORT:-30534}"
EPOCHS="${EPOCHS:-3}"
SCHEDULER_EPOCHS="${SCHEDULER_EPOCHS:-3}"
LR="${LR:-1.5e-5}"
MIN_LR="${MIN_LR:-5e-6}"
QUANT_LR_MULTIPLIER="${QUANT_LR_MULTIPLIER:-2}"
PROGRESSIVE_BIT_SCHEDULE="${PROGRESSIVE_BIT_SCHEDULE:-0:4:8,1:4:6,2:4:4}"
PROGRESSIVE_BIT_RECALIBRATE_EPOCHS="${PROGRESSIVE_BIT_RECALIBRATE_EPOCHS:-1,2}"
...
{
  echo "===== Resume10 smooth activation-curriculum gate W4A8->W4A6->W4A4 $(date '+%F %T') ====="
  echo "QATS=${QATS}"
  echo "DATA=${DATA}"
  echo "OUT=${OUT}/${EXP}"
  echo "RESUME=${RESUME}"
