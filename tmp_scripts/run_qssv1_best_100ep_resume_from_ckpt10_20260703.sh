...
RESUME=/tmp/qat_recipe1_runs/qssv1_best_100ep_fromscratch_20260703/checkpoint-10.pth.tar
LOG=/tmp/train_${EXP}_resume_from_ckpt10_20260703.log
SECONDS=0
echo "resume=$RESUME"
echo "output=${OUT}/${EXP}"
echo "log=$LOG"
PYTHONUNBUFFERED=1 python3 "$QATS/qat_launch.py" \
  ...
  --devices 0,1,2,3,4,5,6,7 --nproc-per-node 8 --master-port 30384 --model-type swin \
  --teacher swin_t --teacher-type swin --teacher-checkpoint "$TEACHER" --teacher-pretrained \
  --epochs 100 --scheduler-epochs 100 --batch-size 256 --workers 8 \
  ...
  --quant-lr-multiplier-epoch-overrides 99:8 \
