...
EXP="${EXP:-recipe2ep_r_fixed_qkr_noaug_scheduler1_quantlr4_true_softkd_temp25_20260705}"
...
{
  echo "===== 2ep recipe R fixed-QKR no-aug scheduler1 quantlr4 true pure-soft-KD temp2.5 $(date '+%F %T') ====="
  echo "QATS=${QATS}"
  echo "DATA=${DATA}"
  echo "OUT=${OUT}/${EXP}"
