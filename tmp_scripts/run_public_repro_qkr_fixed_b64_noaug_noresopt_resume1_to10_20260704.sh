...
EXP="${EXP:-qkr_fixed_b64_noaug_noresopt_resume1_to10_20260704}"
DATA="${DATA:-/tmp/imagenet1k_full_parquet}"
TEACHER="${TEACHER:-/home/tiger/.cache/torch/hub/checkpoints/swin_t-704ceda3.pth}"
RESUME="${RESUME:-/tmp/qat_public_repro/qkr_fixed_b64_noaug_1ep_gate_20260704/checkpoint-1.pth.tar}"
LOG="${LOG:-/tmp/train_${EXP}.log}"
DEVICES="${DEVICES:-0,1,2,3,4,5,6,7}"
MASTER_PORT="${MASTER_PORT:-30437}"
SECONDS=0
...
{
  echo "===== QKR fixed batch64 no-aug no-resume-opt epoch1 to10 $(date '+%F %T') ====="
  echo "QATS=${QATS}"
  echo "DATA=${DATA}"
  echo "OUT=${OUT}/${EXP}"
...
PYTHONUNBUFFERED=1 python3 "${QATS}/qat_launch.py" \
  ...
  --epochs 10 --scheduler-epochs 10 --batch-size 64 --workers 8 \
  --lr 2e-4 --min-lr 1e-5 --weight-decay 0.0 \
  --resume "${RESUME}" --no-resume-opt \
  --epoch-checkpoint-interval 1 --checkpoint-hist 12 \
