...
EXP=swin_t_w4a4_stage1_continue60_to100_lr1e5_tmp_20260701
LOG=/tmp/train_${EXP}.log
OUT=/tmp/qats_stage1_outputs/ofq_prevstep_attn_kl_headtop5_303opt_20260624
CKPT=/mlx_devbox/users/quyanyi/playground/QATs/outputs/ofq_prevstep_attn_kl_headtop5_303opt_20260624/swin_t_w4a4_stage1_resume_diag_continue50_to60_20260701/checkpoint-60.pth.tar
...
PYTHONUNBUFFERED=1 python3 /mlx_devbox/users/quyanyi/playground/QATs/qat_launch.py \
  ...
  --devices 0,1,2,3,4,5,6,7 --nproc-per-node 8 --master-port 30140 \
  --model-type swin --teacher swin_t --teacher-type swin --teacher-checkpoint "${TEACHER}" --teacher-pretrained \
  --resume "${CKPT}" --no-resume-opt --epochs 100 --start-epoch 60 --scheduler-epochs 100 \
  --batch-size 256 --workers 8 --lr 1e-5 --min-lr 1e-5 --weight-decay 0.0 \
