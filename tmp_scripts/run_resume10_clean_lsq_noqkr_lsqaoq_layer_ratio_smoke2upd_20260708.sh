...
EXP="${EXP:-recipe_resume10_clean_lsq_noqkr_lsqaoq_layer_ratio_smoke2upd_20260708}"
...
AOQ_EXPLORE_SCALE_RATIO="${AOQ_EXPLORE_SCALE_RATIO:-0.95}"
...
cmd=(
  ...
  --aoq-explore-layer-ratios "${AOQ_EXPLORE_LAYER_RATIOS}"
...
{
  echo "===== Clean LSQ no-QKR LSQ-AOQ layer-ratio smoke2upd $(date '+%F %T') ====="
  echo "QATS=${QATS}"
  echo "DATA=${DATA}"
  echo "OUT=${OUT}/${EXP}"
  echo "RESUME=${RESUME}"
