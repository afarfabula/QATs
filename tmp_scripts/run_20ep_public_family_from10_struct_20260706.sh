...
EXP="${EXP:-recipe20ep_from10_struct_teacherattn_qkrel_v3_20260706}"
...
{
  echo "===== 20ep continuation from epoch10 80.364 public-style W4A4-family + teacher-attn/QK structure $(date '+%F %T') ====="
  echo "QATS=${QATS}"
  echo "DATA=${DATA}"
  echo "OUT=${OUT}/${EXP}"
  echo "RESUME=${RESUME}"
...
PYTHONUNBUFFERED=1 python3 "${QATS}/qat_launch.py" \
  ...
  --lr 4e-5 --min-lr 1e-5 --weight-decay 0.0 \
  ...
  --teacher-attn-kl-weight 0.00005 \
