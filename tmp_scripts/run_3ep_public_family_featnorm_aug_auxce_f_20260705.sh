...
EXP="${EXP:-recipe3ep_f_featnorm_aug_auxce002_fixed_qkr_softkd_t275_20260705}"
...
{
  echo "===== 3ep recipe F public-style W4A4-family feature-output + augmented teacher distill + auxCE fixed-QKR soft-KD T=2.75 $(date '+%F %T') ====="
  echo "QATS=${QATS}"
  echo "DATA=${DATA}"
  echo "OUT=${OUT}/${EXP}"
...
PYTHONUNBUFFERED=1 python3 "${QATS}/qat_launch.py" \
  ...
  --clean-start-target-loss-weight 0.02 \
