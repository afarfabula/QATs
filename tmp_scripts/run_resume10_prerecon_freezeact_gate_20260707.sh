...
EXP="${EXP:-recipe_resume10_prerecon_freezeact_gate_20260707}"
...
TRAINABLE_POLICY="${TRAINABLE_POLICY:-freeze_act_quant}"
...
{
  echo "===== Resume10 pre-QAT feature recon + freeze activation quant/shift gate $(date '+%F %T') ====="
  echo "QATS=${QATS}"
  echo "DATA=${DATA}"
  echo "OUT=${OUT}/${EXP}"
  echo "RESUME=${RESUME}"
