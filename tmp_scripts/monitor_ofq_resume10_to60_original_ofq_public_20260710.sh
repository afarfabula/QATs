...
EXP="${EXP:-ofq_resume10_to60_original_ofq_public_20260710}"
...
STATUS="${STATUS:-${DOC_DIR}/ofq_resume10_to60_original_ofq_public_status_20260710.tsv}"
REF_STATUS="${REF_STATUS:-${DOC_DIR}/ofq_resume10_to60_original_ofq_public_refw_20260710.tsv}"
SUMMARY="${SUMMARY:-${DOC_DIR}/ofq_resume10_to60_original_ofq_public_monitor_summary_20260710.txt}"

mkdir -p "${DOC_DIR}"
ts="$(date -u '+%Y-%m-%dT%H:%M:%SZ')"

{
  printf 'timestamp\tcheckpoint\tloss\tacc1\tacc5\tsamples\tdelta_vs_80.5980\tdelta_vs_scheme_c_80.6820\tstatus\n'
  if [[ -f "${LOG}" ]]; then
    awk -v ts="${ts}" -v baseline="${BASELINE}" -v scheme_c="${SCHEME_C_BEST}" '
      ...
      END {
        for (i=1; i<=n; i++) {
          ...
          printf "%s\tcheckpoint-%d\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n", ts, ckpt, a[1], a[2], a[3], a[4], delta_base, delta_scheme, status;
        }
      }
    ' "${LOG}"
  fi
