...
EXP="${EXP:-recipe_resume10_clean_lsq_noqkr_lsqaoq090_selectivemargin08_attnanchor_smoke4upd_20260708}" \
LOG="${LOG:-/mlx_devbox/users/quyanyi/playground/train_recipe_resume10_clean_lsq_noqkr_lsqaoq090_selectivemargin08_attnanchor_smoke4upd_20260708.log}" \
MASTER_PORT="${MASTER_PORT:-31143}" \
AOQ_EXPLORE_SELECTIVE_MARGIN="${AOQ_EXPLORE_SELECTIVE_MARGIN:-0.08}" \
AOQ_EXPLORE_QUALITY_MODE="${AOQ_EXPLORE_QUALITY_MODE:-none}" \
AOQ_EXPLORE_END_UPDATE="${AOQ_EXPLORE_END_UPDATE:-2}" \
...
SELECTIVE_BIN_ANCHOR_CAPTURE_UPDATE="${SELECTIVE_BIN_ANCHOR_CAPTURE_UPDATE:-2}" \
SELECTIVE_BIN_ANCHOR_END_UPDATE="${SELECTIVE_BIN_ANCHOR_END_UPDATE:-4}" \
SELECTIVE_BIN_ANCHOR_MARGIN="${SELECTIVE_BIN_ANCHOR_MARGIN:-0.05}" \
MAX_TRAIN_UPDATES="${MAX_TRAIN_UPDATES:-4}" \
