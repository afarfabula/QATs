...
EXP="${EXP:-recipe_resume10_prerecon_actscale_anchor_delayed_gate_20260707}"
...
ACT_SCALE_ANCHOR_WEIGHT="${ACT_SCALE_ANCHOR_WEIGHT:-0.001}"
ACT_SCALE_ANCHOR_LAYERS="${ACT_SCALE_ANCHOR_LAYERS:-features.5.5,features.7.1}"
ACT_SCALE_ANCHOR_START_EPOCH="${ACT_SCALE_ANCHOR_START_EPOCH:-1}"
...
{
  echo "===== Resume10 pre-QAT feature recon + delayed weak activation scale anchor gate $(date '+%F %T') ====="
  echo "QATS=${QATS}"
  echo "DATA=${DATA}"
  echo "OUT=${OUT}/${EXP}"
  echo "RESUME=${RESUME}"
