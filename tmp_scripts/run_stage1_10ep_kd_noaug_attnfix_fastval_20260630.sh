...
EXP=swin_t_w4a4_stage1_bs256_kd_noaug_attncopyfix_10ep_fastval_20260630
...
PYTHONUNBUFFERED=1 python3 "$QATS/qat_launch.py" \
  ...
  --devices 0,1,2,3,4,5,6,7 --nproc-per-node 8 --master-port 29988 --model-type swin \
  --teacher swin_t --teacher-type swin --teacher-checkpoint "$TEACHER" --teacher-pretrained \
  --epochs 10 --scheduler-epochs 50 --batch-size 256 --workers 8 \
