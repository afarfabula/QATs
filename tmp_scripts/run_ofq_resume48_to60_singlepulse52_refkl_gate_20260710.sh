...
MASTER_PORT="${MASTER_PORT:-31731}"
REF_HEAD_MODE="${REF_HEAD_MODE:-custom_subset:6:1,8:4,8:9,11:18,11:4}"
REF_WEIGHT_OVERRIDES="${REF_WEIGHT_OVERRIDES:-52:0.00005}"
SECONDS=0
...
{
  echo "===== OFQ resume48->60 single-pulse52 sparse prev-step refKL gate $(date '+%F %T') ====="
  echo "QATS=${QATS}"
  echo "DATA=${DATA}"
  echo "OUT=${OUT}/${EXP}"
  echo "LOG=${LOG}"
