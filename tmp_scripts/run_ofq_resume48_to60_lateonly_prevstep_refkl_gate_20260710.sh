...
CKPT="${CKPT:-/tmp/qat_public_repro/ofq_resume10_to60_original_ofq_public_20260710/checkpoint-48.pth.tar}"
TEACHER="${TEACHER:-/home/tiger/.cache/torch/hub/checkpoints/swin_t-704ceda3.pth}"
DEVICES="${DEVICES:-0,1,2,3,4,5,6,7}"
MASTER_PORT="${MASTER_PORT:-31721}"
REF_HEAD_MODE="${REF_HEAD_MODE:-custom_subset:6:1,8:4,8:9,11:18,11:4}"
REF_WEIGHT_OVERRIDES="${REF_WEIGHT_OVERRIDES:-49:0.00015,50:0.00020,51:0.00020,52:0.00015,56:0.00010}"
SECONDS=0
...
{
  echo "===== OFQ resume48->60 late-only sparse prev-step refKL gate $(date '+%F %T') ====="
  echo "QATS=${QATS}"
  echo "DATA=${DATA}"
  echo "OUT=${OUT}/${EXP}"
  echo "LOG=${LOG}"
