...
EXP=recipe1_candidate_a_qkr_ofqaug_50ep_20260704
...
# Candidate A follows the older OFQ effective update budget:
# global batch = 64 per GPU * 8 GPUs = 512, about 2502 optimizer updates per ImageNet epoch.
PYTHONUNBUFFERED=1 python3 "$QATS/qat_launch.py" \
  ...
  --devices 0,1,2,3,4,5,6,7 --nproc-per-node 8 --master-port 30406 --model-type swin \
  --teacher swin_t --teacher-type swin --teacher-checkpoint "$TEACHER" --teacher-pretrained \
  --epochs 50 --scheduler-epochs 50 --batch-size 64 --workers 8 \
  --lr 2e-4 --min-lr 1e-5 --weight-decay 0.0 \
  --epoch-checkpoint-interval 10 --checkpoint-hist 8 --val-interval 10 \
