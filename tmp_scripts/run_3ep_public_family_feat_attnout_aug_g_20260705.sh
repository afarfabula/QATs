...
EXP="${EXP:-recipe3ep_g_feat_attnout_aug_fixed_qkr_softkd_t275_20260705}"
...
{
  echo "===== 3ep recipe G public-style W4A4-family feature + attention-output augmented teacher distill fixed-QKR soft-KD T=2.75 $(date '+%F %T') ====="
  echo "QATS=${QATS}"
  echo "DATA=${DATA}"
  echo "OUT=${OUT}/${EXP}"
...
PYTHONUNBUFFERED=1 python3 "${QATS}/qat_launch.py" \
  ...
  --teacher-attn-output-weight 0.001 \
  --teacher-attn-output-layers 6,7,8,9,10,11 \
