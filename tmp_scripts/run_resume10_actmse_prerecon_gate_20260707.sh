...
EXP="${EXP:-recipe_resume10_actmse_prerecon_gate_20260707}"
...
MASTER_PORT="${MASTER_PORT:-30571}"
EPOCHS="${EPOCHS:-3}"
SCHEDULER_EPOCHS="${SCHEDULER_EPOCHS:-3}"
LR="${LR:-1.5e-5}"
MIN_LR="${MIN_LR:-5e-6}"
QUANT_LR_MULTIPLIER="${QUANT_LR_MULTIPLIER:-2}"
PRE_QAT_ACT_MSE_CALIB_BATCHES="${PRE_QAT_ACT_MSE_CALIB_BATCHES:-8}"
PRE_QAT_ACT_MSE_CALIB_LAYERS="${PRE_QAT_ACT_MSE_CALIB_LAYERS:-features.5.5,features.7.1}"
PRE_QAT_ACT_MSE_CALIB_QUANTIZERS="${PRE_QAT_ACT_MSE_CALIB_QUANTIZERS:-features.5.5.attn.quan_a_qkx_fn,features.5.5.attn.quan_a_v_fn,features.5.5.attn.quant_x_4_qkv.input_quant_fn,features.5.5.mlp.fc1.input_quant_fn,features.7.1.attn.quan_a_qkx_fn,features.7.1.attn.quan_a_v_fn,features.7.1.attn.quant_x_4_qkv.input_quant_fn,features.7.1.mlp.fc1.input_quant_fn}"
PRE_QAT_ACT_MSE_CALIB_GRID="${PRE_QAT_ACT_MSE_CALIB_GRID:-0.75,1.05,13}"
PRE_QAT_ACT_MSE_CALIB_BLEND="${PRE_QAT_ACT_MSE_CALIB_BLEND:-0.5}"
...
cmd=(
  ...
  --pre-qat-act-mse-calib-layers "${PRE_QAT_ACT_MSE_CALIB_LAYERS}"
  --pre-qat-act-mse-calib-quantizers "${PRE_QAT_ACT_MSE_CALIB_QUANTIZERS}"
  --pre-qat-act-mse-calib-grid "${PRE_QAT_ACT_MSE_CALIB_GRID}"
  --pre-qat-act-mse-calib-blend "${PRE_QAT_ACT_MSE_CALIB_BLEND}"
...
{
  echo "===== Resume10 selective activation MSE calibration + pre-QAT feature recon gate $(date '+%F %T') ====="
  echo "QATS=${QATS}"
  echo "DATA=${DATA}"
  echo "OUT=${OUT}/${EXP}"
  echo "RESUME=${RESUME}"
