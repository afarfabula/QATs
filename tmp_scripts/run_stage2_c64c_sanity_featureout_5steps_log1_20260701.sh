...
EXP=swin_t_w4a4_stage2_c64c_sanity_featureout_5steps_log1_from50_20260701
LOG=/tmp/${EXP}.log
...
python3 /mlx_devbox/users/quyanyi/playground/QATs/qat_launch.py \
  ...
  --devices 0,1,2,3,4,5,6,7 --nproc-per-node 8 --master-port 30266 \
  ...
  --teacher-feature-output-weight 0.001 \
  --teacher-feature-output-layers features.1,features.3,features.5,features.7 \
  --teacher-feature-output-warmup-epochs 50 \
  --extra-arg=--static-graph \
  --extra-arg=--skip_validate \
  --extra-arg=--max_train_updates --extra-arg=5 \
