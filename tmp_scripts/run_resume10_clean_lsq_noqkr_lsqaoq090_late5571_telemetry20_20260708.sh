...
EXP="${EXP:-recipe_resume10_clean_lsq_noqkr_lsqaoq090_late5571_telemetry20_20260708}"
...
AOQ_EXPLORE_END_UPDATE="${AOQ_EXPLORE_END_UPDATE:-20}"
WEIGHT_BIN_TELEMETRY_LAYERS="${WEIGHT_BIN_TELEMETRY_LAYERS:-features.5.5.attn.qkv,features.5.5.attn.proj,features.5.5.mlp.fc2,features.7.1.attn.qkv,features.7.1.attn.proj,features.7.1.mlp.fc2}"
WEIGHT_BIN_TELEMETRY_START_UPDATE="${WEIGHT_BIN_TELEMETRY_START_UPDATE:-0}"
WEIGHT_BIN_TELEMETRY_END_UPDATE="${WEIGHT_BIN_TELEMETRY_END_UPDATE:-20}"
WEIGHT_BIN_TELEMETRY_INTERVAL="${WEIGHT_BIN_TELEMETRY_INTERVAL:-1}"
WEIGHT_BIN_TELEMETRY_MARGIN="${WEIGHT_BIN_TELEMETRY_MARGIN:-0.05}"
MAX_TRAIN_UPDATES="${MAX_TRAIN_UPDATES:-20}"
...
cmd=(
  ...
  --weight-bin-telemetry-layers "${WEIGHT_BIN_TELEMETRY_LAYERS}"
  --weight-bin-telemetry-start-update "${WEIGHT_BIN_TELEMETRY_START_UPDATE}"
  --weight-bin-telemetry-end-update "${WEIGHT_BIN_TELEMETRY_END_UPDATE}"
  --weight-bin-telemetry-interval "${WEIGHT_BIN_TELEMETRY_INTERVAL}"
  --weight-bin-telemetry-margin "${WEIGHT_BIN_TELEMETRY_MARGIN}"
  ...
  --extra-arg=--max_train_updates --extra-arg="${MAX_TRAIN_UPDATES}"
)
...
{
  echo "===== Clean LSQ no-QKR LSQ-AOQ090 late5571 telemetry20 $(date '+%F %T') ====="
  echo "QATS=${QATS}"
  echo "DATA=${DATA}"
  echo "OUT=${OUT}/${EXP}"
  echo "RESUME=${RESUME}"
  ...
  echo "WEIGHT_BIN_TELEMETRY_LAYERS=${WEIGHT_BIN_TELEMETRY_LAYERS}"
  echo "WEIGHT_BIN_TELEMETRY_START_UPDATE=${WEIGHT_BIN_TELEMETRY_START_UPDATE}"
  echo "WEIGHT_BIN_TELEMETRY_END_UPDATE=${WEIGHT_BIN_TELEMETRY_END_UPDATE}"
  echo "WEIGHT_BIN_TELEMETRY_INTERVAL=${WEIGHT_BIN_TELEMETRY_INTERVAL}"
  echo "WEIGHT_BIN_TELEMETRY_MARGIN=${WEIGHT_BIN_TELEMETRY_MARGIN}"
