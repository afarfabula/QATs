...
EXP="${EXP:-recipe4ep_i_featnorm_aug_fixed_qkr_softkd_t275_20260705}"
...
{
  echo "===== 4ep recipe I public-style W4A4-family feature-output + augmented teacher distill fixed-QKR soft-KD T=2.75 $(date '+%F %T') ====="
  echo "QATS=${QATS}"
  echo "DATA=${DATA}"
  echo "OUT=${OUT}/${EXP}"
...
PYTHONUNBUFFERED=1 python3 "${QATS}/qat_launch.py" \
  ...
  --epochs 4 --scheduler-epochs 4 --batch-size 64 --workers 8 \
