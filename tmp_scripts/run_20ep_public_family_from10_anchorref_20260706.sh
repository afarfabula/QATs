...
EXP="${EXP:-recipe20ep_from10_anchorref_attnkl_qkrel_20260706}"
...
{
  echo "===== 20ep continuation from epoch10 80.364 public-style W4A4-family + anchor-ref attention KL $(date '+%F %T') ====="
  echo "QATS=${QATS}"
  echo "DATA=${DATA}"
  echo "OUT=${OUT}/${EXP}"
  echo "RESUME=${RESUME}"
...
PYTHONUNBUFFERED=1 python3 "${QATS}/qat_launch.py" \
  ...
  --anchor-ref-attn-kl-weight 0.00005 \
  --anchor-ref-warmup-epochs 0 \
