...
EXP="${EXP:-recipe3ep_h_featnorm_aug_qss_fixed_qkr_softkd_t275_20260705}"
...
{
  echo "===== 3ep recipe H public-style W4A4-family feature-output + augmented teacher distill + QSS fixed-QKR soft-KD T=2.75 $(date '+%F %T') ====="
  echo "QATS=${QATS}"
  echo "DATA=${DATA}"
  echo "OUT=${OUT}/${EXP}"
...
PYTHONUNBUFFERED=1 python3 "${QATS}/qat_launch.py" \
  ...
  --epochs 3 --scheduler-epochs 3 --batch-size 64 --workers 8 \
  ...
  --quant-slow-state-policy all \
  --quant-slow-state-observe-start-epoch 2 \
  --quant-slow-state-start-epoch 2 \
