...
EXP="${EXP:-recipe2ep_u_fixed_qkr_noaug_scheduler1_quantlr4_true_softkd_temp2875_20260705}"
...
{
  echo "===== 2ep recipe U fixed-QKR no-aug scheduler1 quantlr4 true pure-soft-KD temp2.875 $(date '+%F %T') ====="
  echo "QATS=${QATS}"
  echo "DATA=${DATA}"
  echo "OUT=${OUT}/${EXP}"
...
PYTHONUNBUFFERED=1 python3 "${QATS}/qat_launch.py" \
  ...
  --pretrained --pretrained-initialized --use-kd --kd-hard-and-soft 0 --teacher-soft-temperature 2.875 \
