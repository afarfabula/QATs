...
EXP="${EXP:-recipe2ep_c_fixed_qkr_noaug_scheduler1_minlr3e5_20260704}"
...
{
  echo "===== 2ep recipe C fixed-QKR no-aug scheduler1 minlr3e-5 $(date '+%F %T') ====="
  echo "QATS=${QATS}"
  echo "DATA=${DATA}"
  echo "OUT=${OUT}/${EXP}"
...
PYTHONUNBUFFERED=1 python3 "${QATS}/qat_launch.py" \
  ...
  --lr 2e-4 --min-lr 3e-5 --weight-decay 0.0 \
