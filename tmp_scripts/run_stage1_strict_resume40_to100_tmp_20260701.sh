...
EXP=swin_t_w4a4_stage1_strict_resume40_to100_tmp_20260701
LOG=/tmp/train_${EXP}.log
OUT=/tmp/qats_stage1_outputs/ofq_prevstep_attn_kl_headtop5_303opt_20260624
CKPT=/mlx_devbox/users/quyanyi/playground/QATs/outputs/ofq_prevstep_attn_kl_headtop5_303opt_20260624/swin_t_w4a4_stage1_bs256_kd_noaug_attncopyfix_continue10_to50_sysdisk_20260630/checkpoint-40.pth.tar
...
PYTHONUNBUFFERED=1 python3 /mlx_devbox/users/quyanyi/playground/QATs/qat_launch.py \
  ...
  --devices 0,1,2,3,4,5,6,7 --nproc-per-node 8 --master-port 30143 \
  --model-type swin --teacher swin_t --teacher-type swin --teacher-checkpoint "${TEACHER}" --teacher-pretrained \
  --resume "${CKPT}" --epochs 100 --scheduler-epochs 100 \
  --batch-size 256 --workers 8 --lr 2e-4 --min-lr 1e-5 --weight-decay 0.0 \
  --epoch-checkpoint-interval 5 --checkpoint-hist 5 \
