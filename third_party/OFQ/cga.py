...
## weight freezing 
parser.add_argument('--boundaryRange', type=float, default=0.005, help="Boubdary Range for CGA, usually 0.005 or 0.007 yield the best result")
parser.add_argument('--freeze_for_n_epochs', type=int, default=15, help="Number of fine-tuning epochs for CGA")
...
def get_qat_model(model, args):
    ...
    if args.model_type == 'deit':	
        qat_model = replace_module_by_qmodule_deit(model, qconfigs, pretrained_initialized = args.pretrained_initialized,
            qk_reparam=args.qk_reparam, qk_reparam_type = args.qk_reparam_type, boundaryRange= args.boundaryRange)	
...
def freeze_outside_boundary_weight_idx( cur_weight, num_bits,boundaryRange = 0.005):
    ...
    def get_min_max_round(weights, scaling_factor, clip_val, num_bits):
        ...
        cliped_weights = torch.clamp(scaled_weights, min=(-clip_val/2), max=(clip_val/2)-1e-6)
        n = float(2 ** (num_bits - 1))
    ...
    scaling_factor = 2 * torch.mean(abs(cur_weight),dim=1,keepdim=True) # dim, 1
    ...
    for i in np.arange(start=min.detach().cpu(),stop=max.detach().cpu()): # 0.5 - boundaryRange < x < 0.5 + boundaryRange
        within_boundary = ((b4_round - i) <= (0.5 + boundaryRange)) *  ((b4_round - i) >= (0.5 - boundaryRange)) #idx of # 0.5 - boundaryRange < x < 0.5 + boundaryRange
        not_freeze_idx  = not_freeze_idx + within_boundary.float()

    return 1.0-not_freeze_idx
...
def main(local_rank, args):
    ...
    args.cooldown_epochs = args.freeze_for_n_epochs
    ...
    ## have to create and init all the parameters before send to optimizer
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        if loss_scaler is not None:
            ...
            if args.qk_reparam:
                if args.model_type == 'swin':
                    for k, v in model.named_modules():
                        if  (k[-3:] == 'fc1' or k[-3:] == 'fc2' or k[-2:] == '.v' or k[-4:] == 'proj' or k[-9:] == 'reduction'):
                            ...
                            v.weight.grad = v.weight.grad * freeze_idx * 0.0 + v.weight.grad * (1 - freeze_idx )
                            
                            save_frozen_weight[k] = (v.weight * freeze_idx).detach().clone()
                else:
                    for k, v in model.named_modules():
                        if 'blocks' in k and (k[-3:] == 'fc1' or k[-3:] == 'fc2' or k[-2:] == '.v' or k[-4:] == 'proj'):
                            freeze_idx = freeze_outside_boundary_weight_idx( v.weight, args.wq_bitw, boundaryRange = args.boundaryRange)
                            freeze_idx_dic[k]= freeze_idx.detach().clone()
                            v.weight.grad = v.weight.grad * freeze_idx * 0.0 + v.weight.grad * (1 - freeze_idx )
                ...
                for k, v in model.named_modules():
                    if 'blocks' in k and (k[-3:] == 'fc1' or k[-3:] == 'fc2' or k[-3:] == 'qkv' or k[-4:] == 'proj'):
                        freeze_idx = freeze_outside_boundary_weight_idx( v.weight, args.wq_bitw, boundaryRange = args.boundaryRange)
                        freeze_idx_dic[k]= freeze_idx.detach().clone()
                        v.weight.grad = v.weight.grad * freeze_idx * 0.0 + v.weight.grad * (1 - freeze_idx )
            ...
            if args.qk_reparam:
                if args.model_type == 'swin':
                    for k, v in model.named_modules():
                        if (k[-3:] == 'fc1' or k[-3:] == 'fc2' or k[-2:] == '.v' or k[-4:] == 'proj' or k[-9:] == 'reduction'):
                            with torch.no_grad():
                                keep_w = (v.weight.detach().clone() * (1-freeze_idx_dic[k]))
                                replace_freeze_part = save_frozen_weight[k]
                                new_weight = keep_w + replace_freeze_part
                                v.weight.data.copy_(new_weight)
                else:
                    for k, v in model.named_modules():
                        if 'blocks' in k and (k[-3:] == 'fc1' or k[-3:] == 'fc2' or k[-2:] == '.v' or k[-4:] == 'proj'):
                            with torch.no_grad():
