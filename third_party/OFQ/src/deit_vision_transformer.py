...
from timm.models.layers import PatchEmbed, DropPath, trunc_normal_, lecun_normal_, to_2tuple
...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.qqkkvv:
            ...
            q_score = q_score / math.sqrt(self.head_dim)
            k_score = torch.matmul(k, k.transpose(-1, -2))
            k_score = k_score / math.sqrt(self.head_dim)
            v_score = torch.matmul(v, v.transpose(-1, -2))
            v_score = v_score / math.sqrt(self.head_dim)
            ...
            x = self.proj(x)
            x = self.proj_drop(x)
            return x, (attn_matrix, q_score, k_score, v_score)
        else:
...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, v_normalized = False, qk_normalized=False, qqkkvv = False, LN_affine = True):
        super().__init__()
        if norm_layer != None:
            self.norm1 = norm_layer(dim, elementwise_affine= LN_affine)
        else:
...
class VisionTransformer(nn.Module):
    ...
    def get_attn_matrix_and_intermediate_features(self, x):
