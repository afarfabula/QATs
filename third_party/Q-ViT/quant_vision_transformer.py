...
class StrictQ_Mlp(nn.Module):
    """Strict quantized MLP with explicit int->dequant activations and weights."""
    def __init__(self, nbits, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0.):
        super().__init__()
        out_features = out_features or in_features
        hidden_features = hidden_features or in_features
        ...
        self.fc1 = StrictLinearQ(in_features, hidden_features, nbits_w=nbits, nbits_a=nbits, input_quant='feature')
        self.act = act_layer()
        self.act_quant = StrictLsqFeatureQuantizer(bit=nbits, all_positive=False, learnable=True)
        self.drop1 = nn.Dropout(drop_probs[0])
        self.fc2 = StrictLinearQ(hidden_features, out_features, nbits_w=nbits, nbits_a=nbits, input_quant='feature')
        self.drop2 = nn.Dropout(drop_probs[1])
...
class StrictQ_Attention(nn.Module):

    def __init__(self, nbits, dim, num_heads=8, qkv_bias=False, attn_drop=0., proj_drop=0.):
        super().__init__()
        assert dim % num_heads == 0, 'dim should be divisible by num_heads'
        self.num_heads = num_heads
        head_dim = dim // num_heads
        ...
        self.qkv = StrictLinearQ(dim, dim * 3, bias=qkv_bias, nbits_w=nbits, nbits_a=nbits, input_quant='feature')
        self.proj = StrictLinearQ(dim, dim, bias=True, nbits_w=nbits, nbits_a=nbits, input_quant='feature')

        self.move_qkv_b4 = LearnableBias(dim * 3)
        self.move_q_aft = LearnableBias(dim)
        self.move_k_aft = LearnableBias(dim)
        self.move_v_b4 = LearnableBias(dim)
        self.move_v_aft = LearnableBias(dim)

        self.q_quant = StrictLsqRowQuantizer(bit=nbits, all_positive=False, learnable=True)
        self.k_quant = StrictLsqRowQuantizer(bit=nbits, all_positive=False, learnable=True)
        self.v_quant = StrictLsqFeatureQuantizer(bit=nbits, all_positive=False, learnable=True)
        self.attn_prob_quant = StrictLsqFeatureQuantizer(bit=nbits, all_positive=True, learnable=True)
    ...
    def forward(self, x):
        ...
        attn = attn.softmax(dim=-1)
        attn = self.attn_prob_quant(attn)
        attn = self.attn_drop(attn)
...
class StrictQ_Block(nn.Module):

    def __init__(self, nbits, dim, num_heads, mlp_ratio=4., qkv_bias=False, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm):
        super().__init__()
        self.norm1 = norm_layer(dim)
        self.attn = StrictQ_Attention(nbits, dim, num_heads=num_heads, qkv_bias=qkv_bias, attn_drop=attn_drop,
                                      proj_drop=drop)
        self.drop_path = DropPath(drop_path) if drop_path > 0. else nn.Identity()
        self.norm2 = norm_layer(dim)
        mlp_hidden_dim = int(dim * mlp_ratio)
        self.mlp = StrictQ_Mlp(nbits=nbits, in_features=dim, hidden_features=mlp_hidden_dim,
...
class StrictQ_PatchEmbed(nn.Module):
    """Strict image-to-patch embedding with explicit int->dequant path."""
    def __init__(self, nbits=4, img_size=224, patch_size=16, in_chans=3, embed_dim=768):
        super().__init__()
        img_size = to_2tuple(img_size)
        patch_size = to_2tuple(patch_size)
        ...
        self.proj = StrictConv2dQ(in_chans, embed_dim, kernel_size=patch_size, stride=patch_size, nbits_w=nbits)
...
class lowbit_VisionTransformer(nn.Module):
    ...
    def __init__(self, nbits, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
                 ...
                 drop_rate=0., attn_drop_rate=0., drop_path_rate=0., embed_layer=Q_PatchEmbed, block_layer=Q_Block,
                 head_layer=LinearQ, head_bits=8, norm_layer=None, act_layer=None, weight_init=''):
        """
        Args:
            nbits: nbits
        ...
        # Classifier head(s)
        self.head = self.head_layer(self.num_features, num_classes, nbits_w=self.head_bits) if num_classes > 0 else nn.Identity()
        # nn.Linear(self.num_features, num_classes) if num_classes > 0 else nn.Identity()
        self.head_dist = None
        if distilled:
            self.head_dist = self.head_layer(self.embed_dim, self.num_classes, nbits_w=self.head_bits) if num_classes > 0 else nn.Identity()
    ...
    def reset_classifier(self, num_classes, global_pool=''):
        self.num_classes = num_classes
        self.head = self.head_layer(self.embed_dim, num_classes, nbits_w=self.head_bits) if num_classes > 0 else nn.Identity()
...
class strict_lowbit_VisionTransformer(lowbit_VisionTransformer):
    def __init__(self, *args, **kwargs):
        kwargs.setdefault('embed_layer', StrictQ_PatchEmbed)
        kwargs.setdefault('block_layer', StrictQ_Block)
        kwargs.setdefault('head_layer', StrictHeadQLinear)
        kwargs.setdefault('head_bits', 8)
...
def _maybe_load_deit_checkpoint(model, url):
...
def _create_strict_deit(variant_name, nbits, embed_dim, depth, num_heads, pretrained=False, pretrained_url='', **kwargs):
    model = strict_lowbit_VisionTransformer(
        nbits=nbits, patch_size=16, embed_dim=embed_dim, depth=depth, num_heads=num_heads, mlp_ratio=4,
        qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    model.default_cfg = default_cfgs.get(variant_name, _cfg())
    if pretrained and pretrained_url:
        _maybe_load_deit_checkpoint(model, pretrained_url)
    return model
...
def fourbits_deit_small_patch16_224(pretrained=False, **kwargs):
    model = lowbit_VisionTransformer(
        nbits=4, patch_size=16, embed_dim=384, depth=12, num_heads=6, mlp_ratio=4, qkv_bias=True,
...
@register_model
def strict_fourbits_deit_tiny_patch16_224(pretrained=False, **kwargs):
    return _create_strict_deit(
        variant_name='vit_deit_tiny_distilled_patch16_224',
        nbits=4,
        embed_dim=192,
        depth=12,
        num_heads=3,
...
@register_model
def strict_threebits_deit_tiny_patch16_224(pretrained=False, **kwargs):
    return _create_strict_deit(
        variant_name='vit_deit_tiny_distilled_patch16_224',
        nbits=3,
        embed_dim=192,
...
@register_model
def strict_twobits_deit_tiny_patch16_224(pretrained=False, **kwargs):
    return _create_strict_deit(
        variant_name='vit_deit_tiny_distilled_patch16_224',
        nbits=2,
        embed_dim=192,
...
@register_model
def strict_fourbits_deit_small_patch16_224(pretrained=False, **kwargs):
    return _create_strict_deit(
        variant_name='vit_deit_small_distilled_patch16_224',
        nbits=4,
        embed_dim=384,
        depth=12,
        num_heads=6,
...
@register_model
def strict_threebits_deit_small_patch16_224(pretrained=False, **kwargs):
    return _create_strict_deit(
        variant_name='vit_deit_small_distilled_patch16_224',
        nbits=3,
        embed_dim=384,
...
@register_model
def strict_twobits_deit_small_patch16_224(pretrained=False, **kwargs):
    return _create_strict_deit(
        variant_name='vit_deit_small_distilled_patch16_224',
        nbits=2,
        embed_dim=384,
