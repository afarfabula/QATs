...
def get_args_parser():
    ...
    parser.add_argument('--data-path', default='/datasets01/imagenet_full_size/061417/', type=str,
                        help='dataset path')
    parser.add_argument('--data-set', default='IMNET', choices=['CIFAR', 'IMNET', 'IMNET_PARQUET', 'IMNET_PARQUET_ITER', 'INAT', 'INAT19'],
...
def main(args):
    ...
    # IterableDataset has no random-access __getitem__, so it must NOT be
    # paired with a sampler; it shards itself across rank/worker internally.
    is_iterable_train = isinstance(dataset_train, torch.utils.data.IterableDataset)
    is_iterable_val = isinstance(dataset_val, torch.utils.data.IterableDataset)
    ...
    if not is_iterable_val:
        if True:  # args.distributed
            if args.dist_eval:
                if len(dataset_val) % utils.get_world_size() != 0:
                    print('Warning: Enabling distributed evaluation with an eval dataset not divisible by process number. '
                          'This will slightly alter validation results as extra duplicate entries are added to achieve '
                          'equal num of samples per-process.')
                sampler_val = torch.utils.data.DistributedSampler(
                    dataset_val, num_replicas=utils.get_world_size(), rank=utils.get_rank(), shuffle=False)
            else:
    ...
    print('sampler_train=', sampler_train, 'iterable=', is_iterable_train)
    ...
    if args.finetune:
        ...
        if 'pos_embed' in checkpoint_model and hasattr(model, 'pos_embed') and hasattr(model, 'patch_embed') and hasattr(model.patch_embed, 'num_patches'):
